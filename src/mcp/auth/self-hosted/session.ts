...
export class Sessions {
  ...
  async challenge(purpose: "login" | "consent" | "logout", sessionId: string | null, binding: string) {
    ...
    await this.db.authFormChallenge.create({
      data: {
        challengeId: challenge.id,
        sessionId,
        purpose,
        binding: this.credentials.hash(binding),
        secretHash: challenge.hash,
  ...
  async consumeChallenge(tx: AuthDb, token: string, purpose: string, sessionId: string | null, binding: string) {
    ...
    const result = await tx.authFormChallenge.updateMany({
      where: {
        challengeId: id,
        purpose,
        sessionId,
  ...
  async login(loginKey: string, challenge: string, binding: string, previousSession?: string) {
    ...
    return this.db.$transaction(async (tx) => {
      ...
      const valid = await tx.authLoginKey.updateMany({
        where: { keyId: key.keyId, revokedAt: null, expiresAt: { gt: new Date() } },
      ...
      if (previousSession) {
        ...
        if (previousId)
          await tx.authSession.updateMany({
            where: { sessionId: previousId, secretHash: this.credentials.hash(previousSession) },
  ...
  async get(token: string, tx: AuthDb = this.db) {
    ...
    if (!session || !this.credentials.matches(token, session.secretHash) || session.user.status !== "enabled")
    ...
    const live = await tx.authSession.updateMany({
      where: {
        ...
        revokedAt: null,
        expiresAt: { gt: now },
        lastSeenAt: { gt: new Date(now.getTime() - DAY / 2) },
      },
      data: { lastSeenAt: now },
    });
