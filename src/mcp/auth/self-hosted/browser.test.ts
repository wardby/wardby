...
describe.skipIf(!process.env.DATABASE_URL)("browser HTTP OAuth flow (database)", () => {
  ...
  it("performs login, consent, exchange, authenticated MCP, refresh/reuse, revocation, and logout over HTTP", async () => {
    ...
    const provider = new SelfHostedAuthProvider({ canonicalUri: origin + "/mcp", signingKey: "a1".repeat(32), credentialHashKey: "b2".repeat(32) }, db);
    const identity = new IdentityService(db, "b2".repeat(32));
    const subject = "browser-" + randomUUID(); subjects.push(subject);
    ...
    mcp.registerTool({ name: "identity", scope: "agents:read", inputSchema: { type: "object" }, handler: async (_, ctx) => ({ content: [{ type: "text", text: ctx.principal.subject }] }) });
    server = await startHttpServer({ mcp, config: { canonicalUri: origin + "/mcp", httpBind: { host: "127.0.0.1", port }, authProviderKind: "self-hosted" }, auth: { authProvider: provider, db, providers }, selfHosted: provider });
    const jar = new Map<string, string>();
    async function visit(path: string, form?: Record<string, string>, extra: Record<string, string> = {}) {
      const res = await fetch(origin + path, { redirect: "manual", method: form ? "POST" : "GET", headers: { cookie: [...jar].map(([k,v]) => `${k}=${v}`).join("; "), ...(form ? { origin, "content-type": "application/x-www-form-urlencoded" } : {}), ...extra }, body: form ? new URLSearchParams(form) : undefined });
      for (const value of res.headers.getSetCookie()) { const [key, token] = value.split(";", 1)[0].split("="); jar.set(key, token); }
      return res;
    }
    const registration = await fetch(origin + "/register", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ redirect_uris: ["https://client.example/callback"], token_endpoint_auth_method: "none" }) });
    expect(registration.status).toBe(201);
    const clientId = ((await registration.json()) as { client_id: string }).client_id; clients.push(clientId);
    const verifier = randomBytes(32).toString("base64url");
    const query = new URLSearchParams({ response_type: "code", client_id: clientId, redirect_uri: "https://client.example/callback", code_challenge_method: "S256", code_challenge: createHash("sha256").update(verifier).digest("base64url"), scope: "agents:read", state: "state", resource: origin + "/mcp" });
    const browser = await chromium.launch({ headless: true, timeout: 10_000, channel: process.env.SECURITY_BROWSER_CHANNEL === "chrome" ? "chrome" : undefined });
    try {
      ...
      expect(cookies.find((c) => c.name === "reevo-dev-session")).toMatchObject({ httpOnly: true, sameSite: "Lax", path: "/" });
      await page.getByRole("button", { name: "Approve" }).click();
      await page.waitForURL("https://client.example/**");
      const browserCode = new URL(page.url()).searchParams.get("code")!;
      const browserToken = await provider.handleToken({ grantType: "authorization_code", clientId, code: browserCode, codeVerifier: verifier, redirectUri: "https://client.example/callback" });
    ...
    expect(signedIn.status).toBe(303); expect(signedIn.headers.get("set-cookie")).toContain("HttpOnly; SameSite=Lax");
    ...
    const exchange = { grant_type: "authorization_code", client_id: clientId, code, code_verifier: verifier, redirect_uri: "https://client.example/callback" };
    const response = await visit("/token", exchange); expect(response.status).toBe(200);
    ...
    expect(JSON.stringify(await client.callTool({ name: "identity", arguments: {} }))).toContain(subject); await client.close();
    const refresh = { grant_type: "refresh_token", client_id: clientId, refresh_token: token.refresh_token };
    const rotated = await visit("/token", refresh); expect(rotated.status).toBe(200);
    ...
    const logout = await visit("/logout"); const logoutCsrf = /name="csrf" value="([^"]+)"/.exec(await logout.text())![1];
