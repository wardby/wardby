...
it.each(["http://public.example/mcp", "https://u:p@host/mcp", "https://host/mcp#x"])(
  "rejects insecure canonical URI %s",
  (url) => expect(() => canonicalUrl(url)).toThrow(),
);
...
it("rejects declared oversized bodies before streaming", async () => {
  await expect(readBody(request({ "content-length": "101" }), 100, new AbortController().signal)).rejects.toMatchObject(
    { status: 413 },
  );
});
it("rejects chunked overflow and accepts exactly the byte limit", async () => {
  const over = request();
  const a = readBody(over, 3, new AbortController().signal);
  (over as unknown as PassThrough).end("four");
  await expect(a).rejects.toMatchObject({ status: 413 });
  const exact = request();
  const b = readBody(exact, 3, new AbortController().signal);
  (exact as unknown as PassThrough).end("abc");
  await expect(b).resolves.toBe("abc");
});
it("aborts an incomplete body and removes listeners", async () => {
  const req = request();
  const abort = new AbortController();
  const p = readBody(req, 100, abort.signal);
  abort.abort();
