...
export class PayloadTooLargeError extends HttpBoundaryError {
  constructor() { super(413, "payload_too_large"); }
}
...
export function canonicalUrl(value: string): URL {
  ...
  if (url.username || url.password || url.hash || url.search || (url.protocol !== "https:" && !(url.protocol === "http:" && loopback))) {
    throw new Error("Canonical URI must be HTTPS (HTTP is allowed only for loopback development), without credentials, query, or fragment.");
  }
  return url;
}
...
export function readBody(req: IncomingMessage, maxBytes: number, signal: AbortSignal): Promise<string> {
  ...
  if (req.headers["content-encoding"] && req.headers["content-encoding"] !== "identity") return Promise.reject(new HttpBoundaryError(415, "unsupported_encoding"));
  return new Promise((resolve, reject) => {
    ...
    const error = (err: Error) => { clean(); req.pause(); reject(err); };
    ...
    const end = () => { clean(); resolve(Buffer.concat(chunks, size).toString("utf8")); };
...
export function parseBody(raw: string, contentType: string | undefined): unknown {
  ...
  if (type === "application/json") {
    try { const value: unknown = JSON.parse(raw); if (value === null || typeof value !== "object") throw new Error(); return value; }
    catch { throw new HttpBoundaryError(400, "invalid_json"); }
  }
  if (type === "application/x-www-form-urlencoded") {
    if (/%(?![a-f\d]{2})/i.test(raw)) throw new HttpBoundaryError(400, "invalid_form");
    const params = new URLSearchParams(raw);
    for (const key of params.keys()) if (params.getAll(key).length !== 1) throw new HttpBoundaryError(400, "duplicate_parameter");
    return params;
  }
