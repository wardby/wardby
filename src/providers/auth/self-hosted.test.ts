...
async function setup() {
  const subject = "security-" + randomUUID();
  ...
  const params = {
    ...
    redirectUri: "https://client.example/cb?existing=1",
    codeChallenge: createHash("sha256").update(verifier).digest("base64url"),
    codeChallengeMethod: "S256",
    resource: uri,
    scope: "agents:read secrets:write unsupported",
    state: "opaque-state",
  };
  async function code() {
...
it("rejects reused credential/signing keys", () =>
  expect(() => new SelfHostedAuthProvider({ ...config, credentialHashKey: signingKey }, db)).toThrow());
...
describe.skipIf(!process.env.DATABASE_URL)("secure self-hosted OAuth (PostgreSQL)", () => {
  ...
  it("provisions a key once through the CLI and lists no credential hashes", async () => {
    const subject = "cli-security-" + randomUUID();
    subjects.push(subject);
    const lines: string[] = [];
  ...
  it.each(["client_secret_post", "client_secret_basic", "private_key_jwt"])(
    "rejects confidential method %s",
    async (method) => {
      await expect(
        provider.registerClient({ redirectUris: ["https://client.example/cb"], tokenEndpointAuthMethod: method }),
      ).rejects.toThrow();
    },
  );
  it.each([
    "http://public.example/cb",
    "https://user:password@client/cb",
    "https://client/cb#fragment",
    "javascript:alert(1)",
  ])("rejects redirect %s", async (redirect) => {
  ...
  it("session identity alone chooses the subject, and only consent yields a code", async () => {
    ...
    expect(await provider.verifyBearer(token.accessToken)).toMatchObject({
      subject: f.subject,
      scopes: ["agents:read", "secrets:write"],
    });
    expect(decodeJwt(token.accessToken)).toMatchObject({
      iss: uri,
      aud: uri,
      sub: f.subject,
  ...
  it("rejects PKCE, client, redirect, resource, and expired code substitution", async () => {
    ...
    for (const overrides of [
      ...
      await expect(provider.handleToken({ ...grant, ...overrides })).rejects.toThrow();
    await db.oAuthAuthorizationCode.update({
      where: { codeId: provider.credentials.id(grant.code, "rva") },
  ...
  it("rejects plain PKCE and exact redirect mismatches", async () => {
    ...
    await expect(
      provider.handleAuthorize({ ...f.params, redirectUri: "https://client.example/cb?existing=2" }),
    ).rejects.toThrow();
  });
  it("denial consumes interaction without issuing code; CSRF cannot be replayed or swapped", async () => {
    const f = await setup();
    const a = await provider.handleAuthorize(f.params);
    const b = await provider.handleAuthorize(f.params);
  ...
  it("stores only hashes and rotates refresh tokens with reuse revoking replacements and access", async () => {
    ...
    const first = await provider.handleToken(code);
    const refresh = {
    ...
    const row = await db.oAuthGrant.findUniqueOrThrow({
      where: { id: provider.credentials.id(first.refreshToken, "rvr") },
    });
  ...
  it("concurrent refresh permits one winner and commits family revocation", async () => {
    const f = await setup();
    const first = await provider.handleToken(await f.code());
    ...
    const family = await db.oAuthFamily.findUniqueOrThrow({
      where: { id: decodeJwt(first.accessToken).fid as string },
    });
  ...
  it("rejects wrong-client refresh without revoking the legitimate family", async () => {
    ...
    const first = await provider.handleToken(await f.code());
    await expect(
      provider.handleToken({ grantType: "refresh_token", clientId: "other", refreshToken: first.refreshToken }),
    ).rejects.toThrow();
  ...
  it("login rejects stolen challenges, replay, key alteration, expiry, revocation, and disabled users", async () => {
    ...
    await expect(
      provider.sessions.login(f.user.loginKey, await provider.sessions.challenge("login", null, "cookie"), "cookie"),
    ).rejects.toThrow();
    const replacement = await identities.createKey(f.subject);
    await identities.revokeKey(provider.credentials.id(replacement, "rvk")!);
    await expect(
      provider.sessions.login(replacement, await provider.sessions.challenge("login", null, "cookie"), "cookie"),
    ).rejects.toThrow();
  ...
  it("rejects tampered, idle, and expired sessions; logout invalidates before reuse", async () => {
    ...
    for (const data of [{ lastSeenAt: new Date(Date.now() - DAY) }, { expiresAt: new Date(0) }]) {
      const s = await provider.sessions.login(
        f.user.loginKey,
        await provider.sessions.challenge("login", null, "new"),
        "new",
      );
  ...
  it("rejects legacy signed tokens even when the old signing key is retained", async () => {
    const legacy = await new SignJWT({ scope: "agents:read" })
      .setProtectedHeader({ alg: "HS256" })
      .setIssuer(uri)
      .setAudience(uri)
      .setSubject("victim")
      .setExpirationTime("1h")
      .sign(Buffer.from(signingKey, "hex"));
    await expect(provider.verifyBearer(legacy)).rejects.toThrow();
  });
  it("shares login throttles across limiter instances", async () => {
    const a = new PostgresRateLimiter(db, provider.credentials);
    const b = new PostgresRateLimiter(db, provider.credentials);
    const id = randomUUID();
