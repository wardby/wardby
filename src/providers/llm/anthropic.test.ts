...
function fakeClient(events: any[]) {
  return { messages: { stream: (_params: any) => (async function* () { for (const e of events) yield e; })() } } as any;
}
async function collect(it: AsyncIterable<LlmStreamEvent>) { const o: LlmStreamEvent[] = []; for await (const e of it) o.push(e); return o; }
...
describe("AnthropicLlmProvider", () => {
  ...
  it("countTokens is offline, inflates over the raw estimate, and counts tools", async () => {
    ...
    const withTools = await p.countTokens("claude-sonnet-5", [{ role: "user", content: "hello world" }], [
