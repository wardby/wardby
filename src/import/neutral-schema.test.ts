...
describe("neutral-schema", () => {
  it("parses a minimal manifest and defaults optional windows to null", () => {
    const m = ManifestSchema.parse({
      ...
      secretMode: "envelope",
      transferKeyId: "bcf0544926dd0b1c",
  ...
  it("parses an envelope secret shell", () => {
    const s = NeutralSecretSchema.parse({
      ...
      description: null,
      ownerEmail: null,
      ciphertext: { v: 1, alg: "x25519-hkdf-sha256-chacha20poly1305-v1", epk: "aa", nonce: "bb", ct: "cc", tag: "dd" },
    });
  ...
  it("parses an agent-tool edge with synthesized capabilities", () => {
    const at = NeutralAgentToolSchema.parse({
      agentName: "a",
      toolName: "fetch-jira",
      allowedSecrets: ["jira_token"],
      allowedDatastorePrefixes: [],
      allowedHosts: ["x.atlassian.net"],
      hostsSource: "firewall-approved",
    });
