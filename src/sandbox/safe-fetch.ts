...
export async function safeFetch(input: string, init: SafeFetchInit = {}, options: SafeFetchOptions = {}) {
  ...
  try {
    ...
    if (body !== undefined && (typeof body !== "string" || Buffer.byteLength(body) > BRIDGE_INPUT_BYTES))
      throw new Error("fetch_body_limit");
    if (
      Object.keys(headers).some((k) =>
        ["host", "connection", "transfer-encoding", "content-length", "upgrade", "proxy-connection"].includes(k),
      )
    )
      throw new Error("fetch_header_invalid");
    ...
    for (let hop = 0; ; hop++) {
        ...
        if (next.origin !== destination.url.origin) {
          ...
          headers = Object.fromEntries(
            Object.entries(headers).filter(([key]) =>
              ["accept", "accept-language", "content-type", "user-agent", "accept-encoding"].includes(key),
            ),
          );
        }
        url = next.href;
      ...
      response.on("data", (chunk: Buffer) => {
        ...
        if (encoded > FETCH_RESPONSE_BYTES) response!.destroy(new Error("fetch_response_limit"));
      });
      ...
      const resultHeaders = Object.fromEntries(
        Object.entries(response.headers)
          .filter(([key]) => !["content-encoding", "content-length", "set-cookie"].includes(key))
      ...
      return {
        ...
        bodyBase64: Buffer.concat(chunks, size).toString("base64"),
      };
    }
  } catch (err) {
