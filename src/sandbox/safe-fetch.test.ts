...
it.each([
  ...
  "fc00::1",
  "ff00::1",
])("blocks special address %s", (address) => expect(isGlobalAddress(address)).toBe(false));
...
it.each([
  "file:///etc/passwd",
  "ftp://public.example",
  "http://u:p@public.example",
  "http://2130706433",
  "http://0x7f000001",
  "http://127.1",
])("rejects alternate or unsafe URL %s", async (url) => await expect(resolveDestination(url)).rejects.toThrow());
it("rejects wildcard/empty allowlist entries and normalizes exact names", () => {
  expect(() => parseAllowedHosts("*.example.com")).toThrow();
  expect(() => parseAllowedHosts("a,,b")).toThrow();
  expect(parseAllowedHosts("Example.COM.")).toEqual(["example.com"]);
});
it.each(["http://127.0.0.1", "http://10.1.1.1", "http://[::1]", "http://169.254.169.254"])(
  "blocks public redirect to %s",
  async (location) => {
...
it("pins the vetted DNS address to both Node lookup calling conventions", async () => {
  ...
  lookup("public.example", {}, callback);
...
it("follows public relative redirects and strips all credentials across origins", async () => {
  const connect = vi
    ...
    .mockResolvedValueOnce(response(302, { location: "https://other.example/" }))
    .mockResolvedValueOnce(response(200, {}, [Buffer.from("ok")]));
...
it("terminates redirect loops", async () => {
  ...
  await expect(safeFetch("https://public.example", {}, { resolve: publicDns, connect })).rejects.toThrow(
    /redirect_limit/,
  );
...
it("bounds streamed and compressed response allocations", async () => {
  for (const zipped of [false, true]) {
    ...
    await expect(
      safeFetch("https://public.example", {}, { resolve: publicDns, connect: async () => stream }),
...
it("cancels while DNS resolution is pending", async () => {
  ...
  const pending = safeFetch(
    "https://public.example",
    {},
    { resolve: () => new Promise(() => {}), signal: abort.signal },
  );
  abort.abort();
