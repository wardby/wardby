...
try {
  ...
  const backup = execFileSync(
    ...
    [
      ...
      database,
      "--schema=" + schema,
      "--exclude-table=" + schema + '."OAuthClient"',
      "--exclude-table=" + schema + '."OAuthGrant"',
      "--no-owner",
      "--no-privileges",
    ],
    { encoding: "utf8" },
  ...
  assert.equal(
    psql(
      "SELECT count(*) FROM information_schema.columns WHERE table_schema='" +
        schema +
        "' AND column_name IN ('clientSecret','refreshToken');",
    ).trim(),
    "0",
  );
  ...
  sql(
    'CREATE TABLE "OAuthClient" ("clientId" TEXT PRIMARY KEY, metadata JSONB NOT NULL, "clientSecret" TEXT, "createdAt" TIMESTAMP(3) DEFAULT CURRENT_TIMESTAMP); CREATE TABLE "OAuthGrant" (id TEXT PRIMARY KEY, "clientId" TEXT, "principalId" TEXT, scope TEXT, "refreshToken" TEXT, "expiresAt" TIMESTAMP(3), "createdAt" TIMESTAMP(3) DEFAULT CURRENT_TIMESTAMP);',
    restore,
  );
  sql(read(latest), restore);
  assert.deepEqual(snapshot(restore), before);
  console.log(
    "PASS: 11 unrelated tables preserved; legacy OAuth rows/columns removed; unrelated-data backup restored and re-migrated without credentials (7 assertions).",
  );
} finally {
