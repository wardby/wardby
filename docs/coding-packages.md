...
Likewise, if the walk is cut short by `REGISTRY_GRAPH_TIMEOUT_MS` or
`REGISTRY_MAX_GRAPH_PACKAGES` before it reaches the requested package, the
package isn't proven absent, so the answer is `503 wardby_graph_incomplete`,
never `wardby_package_not_allowed`. After a timeout the next request resumes
the walk where it stopped (npm retries a 5xx on its own), so a retry can
succeed; after the package bound trips, allowlist the package directly or
raise the bound.
...
| `wardby_graph_incomplete`          | 503    | A lockfile install requested a package the on-demand dependency-graph walk hadn't reached when it was cut short by `REGISTRY_GRAPH_TIMEOUT_MS` (retry: the walk resumes where it stopped) or `REGISTRY_MAX_GRAPH_PACKAGES` (allowlist the package directly, or ask the operator to raise the limit). Never means the package is outside the graph. |
